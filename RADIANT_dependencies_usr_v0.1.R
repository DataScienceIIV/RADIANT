...
# I. Pre-processing ----
  ...
  cranPackages = c("readxl", "ggplot2", "tidyverse", "xlsx", "Rcpp", "RColorBrewer", "matrixStats", "dplyr", "dbplyr", "cowplot", "knitr", 
                   ...
                   "grid", "dbscan", "uwot", "RcppHNSW", "rnndescent", "tsne", "cluster", "clusterCrit", "ggsci", "ash")
  ...
  writeAsinHMapping = function(){
    if (write.ArcsinH.Mapping == TRUE){
      ...
      tempMarkers = as.character(pData(parameters(fs[[1]]))$desc) #index markers in FCS files
      if(length(which(is.na(as.character(pData(parameters(fs[[1]]))$desc)))) > 0){ #index NA entries in markers
        for(i in which(is.na(as.character(pData(parameters(fs[[1]]))$desc)))){
          tempMarkers[i] = pData(parameters(fs[[1]]))$name[i] #replace NA with marker name
        }
      }
      assign("markers", c(colnames(fs)), envir = .GlobalEnv)
      ...
      if(ashPer == "global"){
        ...
        ashDateList = vector(length = length(fs))
        for(i in 1:length(fs)){
          ashDateList[i] = as.character(keyword(fs[[i]])$"$DATE")
        }
        ashDateList = unique(ashDateList)
        assign("ashDateList", ashDateList, envir = .GlobalEnv)
        for(i in unique(ashDateList)){
          assign("asm", file.path(wd, cellType, sprintf("%s %s Catalyst %s ArcSinH mapping.xlsx", i, expID, cellType)), envir = .GlobalEnv)
          write.xlsx(data.frame(index_values = 1:length(markers), fcs_colname = markers, antigen=tempMarkers, run_asinh=vector(length=length(markers)), cofactor = rep(200, length(markers))), asm, row.names = FALSE)
        }
  ...
  cleanAndTransform = function(){
    ...
    if (cleaningMethod == "None" & transformationMethod == "None"){
      ...
      for (file in files){
        ...
        }else if(transformationMethod == "ArcSinH"){ #aaply ArcsinH transformation to reference file
          ff_t = ff_c
          if(ashPer == "global"){
            asinh.chnl.list = c(asinhMap$fcs_colname)[which(asinhMap$run_asinh == TRUE)]
          }else if(ashPer == "date"){ # import ArcsinH mapping file based on date of the reference file
            dateFile = as.character(keyword(ff_t)$"$DATE")
            assign("asm", file.path(wd, cellType, sprintf("%s %s Catalyst %s ArcSinH mapping.xlsx", dateFile, expID, cellType)), envir = .GlobalEnv)
            message("Attaching ArcsinH mapping file of ", dateFile, "to reference file")
            asinhMap = read.xlsx(asm,1, stringsAsFactors=FALSE) 
            assign("asinhMap", asinhMap, envir = .GlobalEnv)
            asinh.chnl.list = c(asinhMap$fcs_colname)[which(asinhMap$run_asinh == TRUE)]
          }
          message("Reading ArcSinH cofactors")
        ...
        if(transformationMethod == "Logicle"){
          ...
          if(ashPer == "global"){
            ...
            dateFile = as.character(keyword(ff_t)$"$DATE")
            assign("asm", file.path(wd, cellType, sprintf("%s %s Catalyst %s ArcSinH mapping.xlsx", dateFile, expID, cellType)), envir = .GlobalEnv)
            message("Attaching ArcsinH mapping file of ", dateFile, "to sample file")
            asinhMap = read.xlsx(asm,1, stringsAsFactors=FALSE) 
            assign("asinhMap", asinhMap, envir = .GlobalEnv)
            asinh.chnl.list = c(asinhMap$fcs_colname)[which(asinhMap$run_asinh == TRUE)]
          }
  ...
  run_CE = function() {
    ...
    
    {
      ...
      else
      {
        ...
        umap.result <- umap::umap( dmrd.data, config = umap.config )
...
# II. FlowSOM clustering ----
  ...
  performClustering = function(){
    ...
    sce <- cluster_new(sce, features = type_markers(sce), xdim = FSxdim, ydim=FSydim, maxK=metClust, seed=seedFS) ##xdim * ydim = total SOM clusters; maxK = total metaclusters
    ...
    if(performPG == TRUE){
      ...
      assign("phenographFile" , Rphenograph_out, envir = .GlobalEnv)
      ...
      assign("exprsDataPG" , exprsData, envir = .GlobalEnv)
  ...
  performDimRed = function(){
    ...
    if ("T-SNE" %in% anType){
      sce <- runDR(sce, dr=c("TSNE"), cells = TSNE_cells, features=type_markers(sce))
      assign("sce" , sce, envir = .GlobalEnv)
    }
  }
  
  generateCellDataOld = function(){
    cellData = as.data.frame(colData(sce))
    for(i in reducedDimNames(sce)){ #append UMAP and or TNSE coordinates to dataframe
      tempdf = as.data.frame(reducedDim(sce, i))
      if(i == "UMAP"){
        colnames(tempdf) = c("UMAP_1", "UMAP_2")
        cellData = cbind(cellData, tempdf)
      }else if(i == "TSNE"){
        colnames(tempdf) = c("TSNE_1", "TSNE_2")
        cellData = cbind(cellData, tempdf)
      }
    }
    
    MCList = list()
    for(i in names(cluster_codes(sce))){ #append metacluster IDs to dataframe
      MCList[[i]] = cluster_ids(sce, i)
    }
    cellData = cbind(cellData, as.data.frame(MCList))
    
    #append phenograph cluster IDs to dataframe
    if(performPG == TRUE){
      cellData$PG_clusterIDs = phenographFile[[2]]$membership
      cellData$PG_clusterIDs = factor(cellData$PG_clusterIDs, levels = c(unique(cellData$PG_clusterIDs)))
    }
    
    #append raw expression data to dataframe
    cellData = cbind(cellData, t(as.data.frame(assay(sce, "exprs"))))
    
    #append scaled expression to dataframe
    scaledExprs = .scale_exprs_new(as.matrix(assay(sce, "exprs")), 1, 0.01)
    scaledExprs = t(as.data.frame(scaledExprs))
    colnames(scaledExprs) = paste0(colnames(scaledExprs),"_scaled")
    cellData = cbind(cellData, scaledExprs)
    
    write_rds(cellData, paste0(rsl, "/cellData.rds"))
  }
  ...
  generateCellData = function(){
    ...
    # Append reduced dimensions (UMAP and TSNE only)
    dimNames <- reducedDimNames(sce)
    dimMap <- list(UMAP = c("UMAP_1", "UMAP_2"), TSNE = c("TSNE_1", "TSNE_2"))
    
    for (name in intersect(names(dimMap), dimNames)) {
      temp <- as.data.frame(reducedDim(sce, name))
      colnames(temp) <- dimMap[[name]]
      cellData <- bind_cols(cellData, temp)
    }
    
    # Append metacluster IDs
    mcData <- lapply(names(cluster_codes(sce)), function(i) cluster_ids(sce, i))
    names(mcData) <- names(cluster_codes(sce))
    cellData <- bind_cols(cellData, as.data.frame(mcData))
    
    # Append Phenograph cluster IDs
    if (performPG) {
      cellData$PG_clusterIDs <- factor(phenographFile[[2]]$membership,
                                       levels = unique(phenographFile[[2]]$membership))
    }
    
    # Append raw expression data
    exprs_raw <- t(assay(sce, "exprs"))
    cellData <- bind_cols(cellData, as.data.frame(exprs_raw))
    
    # Append scaled expression data
    scaled <- .scale_exprs_new(as.matrix(assay(sce, "exprs")), 1, 0.01)
    scaled <- t(scaled)
    colnames(scaled) <- paste0(colnames(scaled), "_scaled")
    cellData <- bind_cols(cellData, as.data.frame(scaled))
    
    # Save to disk
    write_rds(cellData, file.path(rsl, "cellData.rds"))
  }
  
  
  performDC = function(){
    if(runDC == TRUE){
      if(("PhenoGraph" %in% DCmethod | "FlowSOM" %in% DCmethod)){
        print(Sys.time())
        print("Initialising DeepClustering")
        cellData = read_rds(paste0(rsl, "/cellData.rds")) #import cellData
        
        #create directories
        dir.create(paste0(rsl,"/deepClustering"))
        dcr = paste0(rsl,"/deepClustering")
        
        dir.create(paste0(rsl,"/deepClustering/FlowSOM"))
        dir.create(paste0(rsl,"/deepClustering/FlowSOM/Surface_clustering"))
        dir.create(paste0(rsl,"/deepClustering/FlowSOM/Deep_clustering"))
        dcfssc = paste0(rsl,"/deepClustering/FlowSOM/Surface_clustering")
        dcfsdc = paste0(rsl,"/deepClustering/FlowSOM/Deep_clustering")
        
        
        dir.create(paste0(rsl,"/deepClustering/PhenoGraph"))
        dir.create(paste0(rsl,"/deepClustering/PhenoGraph/Surface_clustering"))
        dir.create(paste0(rsl,"/deepClustering/PhenoGraph/Deep_clustering"))
        dcpgsc = paste0(rsl,"/deepClustering/PhenoGraph/Surface_clustering")
        dcpgdc = paste0(rsl,"/deepClustering/PhenoGraph/Deep_clustering")
        
        
        DCitrFS = 0
        DCitrPG = 0
      }else{
        stop("Invalid DCmethod")
      }
      
      
      
      if(!any(grepl("UMAP", colnames(cellData)))){ #check if cellData contains UMAP coordinates
        stop("This function only works if UMAP has been performed. Make sure to include UMAP in anType above")
      }
      
      print("Initiating Surface Clustering")
      cellDataCND = na.omit(cellData) #create df containing only the dimRed Data
      dimRedCoord = (cellDataCND)[,grep("UMAP", colnames(cellData))] #create df containing only UMAP coords
      #dbScan = dbscan(dimRedCoord, eps = 0.5) #cluster based on UMAP coords
      #cellDataCND$dbScan = dbScan$cluster #append dbscan clusters to master df
      cellDataCND$index = c(1:nrow(cellDataCND))
      
      
      ##create binMatrix of UMAP
      print("Creating bin matrix")
      binRange = rbind(c(min(cellDataCND$UMAP_1)*1.05, max(cellDataCND$UMAP_1)*1.05), c(min(cellDataCND$UMAP_2)*1.05, max(cellDataCND$UMAP_2)*1.05))
      binMtx  = as.data.frame(t(bin2(cbind(cellDataCND$UMAP_1, cellDataCND$UMAP_2), ab = binRange,  nbin = c(binNo,binNo))$nc))
      binMtx2 = binMtx
      binMtx2$index = sort(c(1000:(1000+(nrow(binMtx2)-1))), decreasing = TRUE)
      binMtx2 = binMtx2[ order(binMtx2$index), ]
      binMtx2 = binMtx2[,1:(ncol(binMtx2)-1)]
      row.names(binMtx2) = 1:binNo
      
      print("Successfully binned data. Creating Heatmap")
      #print(pheatmap(binMtx2, cluster_rows = FALSE, cluster_cols = FALSE))
      pheatmap(binMtx2, cluster_rows = FALSE, cluster_cols = FALSE, filename = paste0(dcr, "/Heatmap of bins.png"))
      #png(filename = paste0(dcr, "/Heatmap of bins.png"), width = 3000, height = 3000, res = 300)
      #print(pheatmap(binMtx2, cluster_rows = FALSE, cluster_cols = FALSE))
      #dev.off
      
      
      ##Obtain UMAP coordinates of center points of each bin
      print("Obtaining UMAP coordinates of bins")
      binMtx2 = t(apply(binMtx2, 2, rev)) #transpose bin matrix to match UMAP plot
      binVals = which(binMtx2 != 0, arr.ind = TRUE) #obtain coordinates of bins containing cells
      binVals = data.frame(binVals) #convert matrix to dataframe
      colnames(binVals) = c("col", "row")
      minBinUMAP_1 = min(cellDataCND$UMAP_1)*1.05 #obtain minimal value of UMAP1 (bin2 automatically extends with 5%)
      maxBinUMAP_1 = max(cellDataCND$UMAP_1)*1.05 #obtain maximum value of UMAP1 (bin2 automatically extends with 5%)
      minBinUMAP_2 = min(cellDataCND$UMAP_2)*1.05 #obtain minimal value of UMAP2 (bin2 automatically extends with 5%)
      maxBinUMAP_2 = max(cellDataCND$UMAP_2)*1.05 #obtain maximum value of UMAP2 (bin2 automatically extends with 5%)
      binWidthUMAP_1 = (maxBinUMAP_1-minBinUMAP_1)/binNo #calculate the bin width of UMAP1
      binWidthUMAP_2 = (maxBinUMAP_2-minBinUMAP_2)/binNo #calculate the bin width of UMAP2
      binUMAP_1 = (((binVals$col)*binWidthUMAP_1)+minBinUMAP_1)-(0.5*binWidthUMAP_1) #calculate the UMAP1 coordinate of the center of the bin
      binUMAP_2 = (((binVals$row)*binWidthUMAP_2)+minBinUMAP_2)-(0.5*binWidthUMAP_2) #calculate the UMAP2 coordinate of the center of the bin
      binVals$binUMAP_1 = binUMAP_1 #append UMAP1 to dataframe
      binVals$binUMAP_2 = binUMAP_2 #append UMAP2 to dataframe
      
      ##plot UMAP coordinates of bins
      print("Successfully obtained UMAP coordinates of bins. Plotting results")
      n = ggplot(as.data.frame(binVals), aes(x = binUMAP_1, y = binUMAP_2))+
        geom_point()+
        ggtitle("UMAP of bins")
      
      print(n)
      ggsave(paste0(dcr, "/UMAP_bins.png"), device = "png")
      #ylim(min(cellDataCND$UMAP2),max(cellDataCND$UMAP2))+
      #xlim(min(cellDataCND$UMAP1),max(cellDataCND$UMAP1))
      
      ##Perform dbScan clustering
      print("Performing dbScan on UMAP coordinates of bins")
      dbScan = dbscan(cbind(binVals$binUMAP_1, binVals$binUMAP_2), eps = dbScanEps, minPts = dbScanMinPts) #cluster on UMAP coordinates of bins
      print(paste0("Number of dbScan clusters: ", length(unique(dbScan$cluster))))
      binVals$dbScan = factor(dbScan$cluster, levels = c(1:length(unique(dbScan$cluster)))) #append clusterIDs to dataframe
      
      ##Plot dbScan clusters of bins
      print("Finished dbScan clustering. Plotting results")
      n = ggplot(binVals, aes(x = binUMAP_1, y = binUMAP_2, color = dbScan))+
        geom_point()+
        scale_color_ucscgb()+
        ggtitle("UMAP of dbScan clustered bins")
      
      print(n)
      ggsave(paste0(dcr, "/dbScan_bins.png"), device = "png")
      
      ##Calculate in which bin each cell would fall
      print("Assigning bin clusterIDs to cells in bins")
      ix = floor((cellDataCND$UMAP_1 - minBinUMAP_1)/binWidthUMAP_1) + 1
      iy = floor((cellDataCND$UMAP_2 - minBinUMAP_2)/binWidthUMAP_2) + 1
      #iy = c(rep(100,length(iy)))-iy
      calcBinID = paste(iy,ix,sep = "_")
      indeces_per_bin = split(seq_along(cellDataCND$UMAP_1), calcBinID)
      
      actualBinID = paste(binVals$row, binVals$col, sep = "_")
      
      bin_dbScan = c(rep(NA, nrow(cellDataCND)))
      pb = txtProgressBar(min = 0, max = nrow(binVals), initial = 0)
      for(dbID in 1:nrow(binVals)){
        #print(paste0("Processing bin " , dbID, " of ", nrow(binVals)))
        setTxtProgressBar(pb,dbID)
        bin_dbScan[indeces_per_bin[[actualBinID[dbID]]]] = binVals$dbScan[dbID]
      }
      close(pb)
      
      ##Append cluster_IDs to master dataframe
      cellDataCND$dbScan = bin_dbScan
      cellDataCND$dbScan = factor(cellDataCND$dbScan, levels = c(1:length(bin_dbScan)))
      
      
      
      #plot UMAP with dbScan clusters
      print("Finished assigning bin clusterIDs to cells. Plotting results")
      
      if(plotFraction != FALSE){
        tempPlotDf = data.frame()
        for(fdbscan in unique(cellDataCND$dbScan)){
          if(nrow(subset(cellDataCND, dbScan == fdbscan)) >= plotFraction){
            tempPlotDf = rbind(tempPlotDf, subset(cellDataCND, dbScan == fdbScan)[plotFraction,])
          }else{
            tempPlotDf = rbind(tempPlotDf, subset(cellDataCND, dbScan == fdbScan))
          }
          n = ggplot(tempPlotDf, aes(x=UMAP_1, y=UMAP_2, color = dbScan))+
            geom_point(alpha = 0.6, size = 0.6) +
            labs(title = paste("UMAP of total dataset"),
                 x = "UMAP_1", y = "UMAP_2") +
            theme_classic(base_size = 14)+
            guides(color = guide_legend(override.aes = list(size = 3)))
          
          if(length(unique(as.integer(cellDataCND$dbScan)))<27){
            n = n + scale_color_ucscgb()
          }else{
            n = n + scale_color_igv()
          }
          
          print(n)
          ggsave(paste0(dcr, "/Global_UMAP_dbScan_clusters.png"), device = "png")
        }
        
      }else{
        n = ggplot(cellDataCND, aes(x=UMAP_1, y=UMAP_2, color = dbScan))+
          geom_point(alpha = 0.6, size = 0.6) +
          labs(title = paste("UMAP of total dataset"),
               x = "UMAP_1", y = "UMAP_2") +
          theme_classic(base_size = 14)+
      ...
      #plot UMAP with scaled expression
      for(mrk in colnames(cellDataCND)[which(grepl("scaled", colnames(cellDataCND)))]){
        n = ggplot(cellDataCND, aes_string(x="UMAP_1", y="UMAP_2", color = mrk))+
          geom_point(alpha = 0.6, size = 0.6) +
          labs(title = paste0("UMAP on total dataset - ",mrk),
               x = "UMAP_1", y = "UMAP_2") +
          theme_classic(base_size = 14)+
          #guides(color = guide_legend(override.aes = list(size = 3)))+
          scale_color_viridis_c()
        print(n)
        ggsave(paste0(dcr, "/Global_UMAP_", mrk,".png"), device = "png")
      }
      
      #calculate NRS
      globalDimRedMarkers = which(colnames(cellDataCND) %in% type_markers(sce)) #isolate index values containing the type_markers and add to list
      globalDimRedMarkers = append(globalDimRedMarkers, which(colnames(cellDataCND) %in% state_markers(sce))) #append state_markers to list
      
      #calculate non-redundancy score per dbscan cluster
      masterNRSscores = setNames(data.frame(matrix(ncol = 3, nrow = 0)), c("marker", "NRSscore", "dbScan"))
      for(i in sort(unique(cellDataCND$dbScan))){
        cat("\n")
        print(paste("Analysing dbScan cluster",i))
        cat("\n")
        if(nrow(subset(cellDataCND, dbScan == i)[,globalDimRedMarkers]) < DCminCells){
          print("Amount of cells in this cluster is less than indicated lower limit. Skipping deepclustering")
          if("FlowSOM" %in% DCmethod){
            cellDataCND$dbScan_FS[cellDataCND$dbScan == i] = as.numeric(rep(1 + DCitrFS, length(cellDataCND$dbScan[cellDataCND$dbScan == i])))
            DCitrFS = DCitrFS + 1
          }
          
          if("PhenoGraph" %in% DCmethod){
            cellDataCND$dbScan_PG[cellDataCND$dbScan == i] = as.numeric(rep(1 + DCitrPG, length(cellDataCND$dbScan[cellDataCND$dbScan == i])))
            cellDataCND$dbScan_PG = cellDataCND$dbScan_PG
            DCitrPG = DCitrPG + 1
          }
        ...
        #pr <- prcomp(subset(cellDataCND, dbScan == i)[,globalDimRedMarkers], 
                     #center = TRUE, scale. = FALSE)
        #NRSscore <- rowSums(outer(rep(1, length(globalDimRedMarkers)),
                                  #pr$sdev[1:length(globalDimRedMarkers)]^2) * abs(pr$rotation[,1:length(globalDimRedMarkers)]))
        
        pr <- prcomp(subset(cellDataCND, dbScan == i)[,globalDimRedMarkers], 
                     center = TRUE, scale. = FALSE)
        colsCD = ncol(subset(cellDataCND, dbScan == i)[,globalDimRedMarkers])
        NRSscore <- rowSums(outer(rep(1, colsCD),
                                  pr$sdev[1:3]^2) * abs(pr$rotation[,1:3]))
        
        NRSscoredf = tibble::rownames_to_column(as.data.frame(NRSscore), "marker") #store NRS values in df
        NRSscoredf$dbScan = rep(i, nrow(NRSscoredf))
        masterNRSscores = rbind(masterNRSscores, NRSscoredf)
        NRSscoredf = subset(NRSscoredf, NRSscore > NRSthreshold) #filter markers with NRS scores above threshold
        
        #perform clustering on dbscan clusters with markers of interest
        deepClusterDimRedMarkers = which(colnames(cellDataCND) %in% NRSscoredf$marker) #store indeces of markers with NRS above threshold
        if(length(deepClusterDimRedMarkers) < 2){ #if there are no non-redundant markers go to next iteration
          print("All markers are redundant, or only 1 marker non-redundant. Skipping deep clustering")
        ...
        deepClusterdf = subset(cellDataCND, dbScan == i)
        deepClusterExprsdf = subset(cellDataCND, dbScan == i)[,deepClusterDimRedMarkers] #create df containing markers of interest for deep clustering
        
        if("FlowSOM" %in% DCmethod){
          ff_deepCluster = flowFrame(as.matrix(deepClusterExprsdf)) #create flowframe
          set.seed(123)
          fsom <- FlowSOM(ff_deepCluster, #run flowsom
                          xdim = 10, ydim = 10,
                          colsToUse = 1:ncol(deepClusterExprsdf),
                          scale = FALSE)
          
          # === 7. Try multiple k (10:50) and compute silhouette ===
          k_range <- 5:50 #select range of MCs to compute silhouette for
          sil_deepCluster <- numeric(length(k_range))
          subset_idx2 <- sample(1:nrow(deepClusterExprsdf), min(5000, nrow(deepClusterExprsdf)))
          deepCluster_subset <- deepClusterExprsdf[subset_idx2, ]
          
          for (kri in seq_along(k_range)) {
            k <- k_range[kri]
            meta <- metaClustering_consensus(fsom$map$codes, k = k)
            clust_labels <- meta[fsom$map$mapping[, 1]][subset_idx2]
            sil <- silhouette(clust_labels, dist(deepCluster_subset))
            sil_deepCluster[kri] <- mean(sil[, 3])
          }
          
          # === 8. Auto-select best k for the zoomed-in region ===
          select_optimal_k <- function(k_range, sil_scores, tolerance = 0.01) {
            deltas <- diff(sil_scores)
            drop_points <- which(deltas < -tolerance)
            if (length(drop_points) == 0) {
              return(k_range[which.max(sil_scores)])  # fallback
            } else {
              return(k_range[drop_points[1]])  # first significant drop
            }
          }
          
          optimal_k <- select_optimal_k(k_range, sil_deepCluster)
          meta_best <- metaClustering_consensus(fsom$map$codes, k = optimal_k)
          
          #deepClusterdf$deepCluster_meta <- meta_best[fsom$map$mapping[, 1]]
          deepClusterdf$dbScan_FS <- (meta_best[fsom$map$mapping[, 1]] + DCitrFS)
          DCitrFS = DCitrFS + optimal_k
          #cellDataCND$deepCluster_meta <- NA
          cellDataCND$dbScan_FS[cellDataCND$dbScan == i] = deepClusterdf$dbScan_FS
          deepClusterdf$dbScan_FS = factor(deepClusterdf$dbScan_FS)
          
          # === Plot silhouette score for the zoomed-in population ===
          df_sil_deepCluster <- data.frame(k = k_range, silhouette = sil_deepCluster)
          
          p4 <- ggplot(df_sil_deepCluster, aes(x = k, y = silhouette)) +
            geom_line(color = "steelblue", size = 1) +
            geom_point(color = "darkred", size = 2) +
            geom_vline(xintercept = optimal_k, linetype = "dashed", color = "gray40") +
            labs(title = paste0("Silhouette Score dbScan cluster ",i),
                 x = "Number of Clusters (k)", y = "Avg Silhouette Score") +
            theme_classic(base_size = 14)
          print(p4)
          ggsave(paste0(dcfssc,"/Silhouette score dbScanCluster_",i,".png"), device = "png")
          
          # === UMAP colored by final subclusters ===
          tempumapdf = as.data.frame(umap2(deepClusterExprsdf))
          colnames(tempumapdf) = c("SC_UMAP_1", "SC_UMAP_2")
          deepClusterdf$SC_UMAP_1 = tempumapdf[["SC_UMAP_1"]]
          deepClusterdf$SC_UMAP_2 = tempumapdf[["SC_UMAP_2"]]
          cellDataCND$SC_UMAP_1[cellDataCND$dbScan == i] = tempumapdf$SC_UMAP_1
          cellDataCND$SC_UMAP_2[cellDataCND$dbScan == i] = tempumapdf$SC_UMAP_2
          
          p5 <- ggplot(deepClusterdf, aes(x = SC_UMAP_1, y = SC_UMAP_2, color = dbScan_FS)) +
            geom_point(alpha = 0.6, size = 0.6) +
            #scale_color_viridis_d(name = "Subcluster") +
            labs(title = paste("UMAP on dbScan cluster ",i, " - optimal k ", optimal_k),
                 x = "SC_UMAP_1", y = "SC_UMAP_2") +
            theme_classic(base_size = 14)+
            guides(color = guide_legend(override.aes = list(size = 3)))
          
          if(length(unique(as.integer(deepClusterdf$dbScan_FS)))<27){
            p5 = p5 + scale_color_ucscgb()
          }else{
            p5 = p5 + scale_color_igv()
          }
            
          print(p5)
          ggsave(paste0(dcfssc,"/UMAP dbScanCluster_",i,"_FlowSOM.png"), device = "png")
        }
        
        if("PhenoGraph" %in% DCmethod){
          Rphenograph_out <- Rphenograph(deepClusterExprsdf, k = PGnn) # perform phenograph
          PGclusters = as.numeric(membership(Rphenograph_out[[2]]) + DCitrPG)
          deepClusterdf$dbScan_PG = PGclusters
          DCitrPG = DCitrPG + max(membership(Rphenograph_out[[2]]))
          
          cellDataCND$dbScan_PG[cellDataCND$dbScan == i] = deepClusterdf$dbScan_PG
          deepClusterdf$dbScan_PG = factor(deepClusterdf$dbScan_PG)
          ...
          p6 <- ggplot(deepClusterdf, aes(x = SC_UMAP_1, y = SC_UMAP_2, color = dbScan_PG)) +
            geom_point(alpha = 0.6, size = 0.6) +
            #scale_color_viridis_d(name = "Subcluster") +
            labs(title = paste("UMAP on dbScan cluster ",i),
                 x = "SC_UMAP_1", y = "SC_UMAP_2") +
            theme_classic(base_size = 14)+
            guides(color = guide_legend(override.aes = list(size = 3)))
          
          if(length(unique(as.integer(deepClusterdf$dbScan_PG)))<27){
            p6 = p6 + scale_color_ucscgb()
          }else{
            p6 = p6 + scale_color_igv()
          }
            
          print(p6)
          ggsave(paste0(dcpgsc, "/UMAP dbScanCluster_",i,"_PhenoGraph.png"), device = "png")
        }
      }
      
      #if("FlowSOM" %in% DCmethod){
      # cellDataCND$dbScan_FS = factor(cellDataCND$dbScan_FS)
      #}
      #if("PhenoGraph" %in% DCmethod){
      #  cellDataCND$dbScan_PG = factor(cellDataCND$dbScan_PG)
      #}
      
      
      write.xlsx(masterNRSscores, paste0(dcr,"/NRSscores surface clustering.xlsx"))
      print("Finished Surface Clustering")
      print("Initiating Deep Clustering")
      
      DCcounter = 0
      DCround = 0
      if("FlowSOM" %in% DCmethod){ #create a list of the current FlowSOM cluster IDs
        DCitrFS = 0
        DCidsFS = as.numeric(cellDataCND$dbScan_FS) #create a list of the current FlowSOM cluster IDs
        cellDataCND$DC_IDs_FS = DCidsFS #create a column with the to-be final FlowSOM cluster IDs
        cellDataCND$temp = DCidsFS #create a temporary column containing the cluster IDs after each while loop
        while(DCcounter != max(DCidsFS)){
          DCround = DCround + 1
          print(paste("Deep Clustering round", DCround))
          DCcounter = 0
          masterNRSscores = setNames(data.frame(matrix(ncol = 3, nrow = 0)), c("marker", "NRSscore", "DC_IDs_PG"))
          
          for(i in unique(cellDataCND$DC_IDs_FS)[order(unique(cellDataCND$DC_IDs_FS))]){
            cat("\n")
            print(paste("Analysing FlowSOM cluster ",i, " of ", max(DCidsFS)))
            cat("\n")
            if(nrow(subset(cellDataCND, DC_IDs_FS == i)[,globalDimRedMarkers]) < DCminCells){ #if the minimum amount of cells in this cluster is not met deep clustering is skipped
              print("Amount of cells in this cluster is less than indicated lower limit. Skipping deepclustering")
              
              cellDataCND$temp[cellDataCND$DC_IDs_FS == i] = as.numeric(rep(1 + DCitrFS, length(cellDataCND$temp[cellDataCND$DC_IDs_FS == i]))) #append current FS cluster ID to temp column
              DCitrFS = DCitrFS + 1
            ...
            #Calculate NRS scores
            #pr <- prcomp(subset(cellDataCND, DC_IDs_FS == i)[,globalDimRedMarkers], 
                         #center = TRUE, scale. = FALSE)
            #NRSscore <- rowSums(outer(rep(1, length(globalDimRedMarkers)),
                                      #pr$sdev[1:length(globalDimRedMarkers)]^2) * abs(pr$rotation[,1:length(globalDimRedMarkers)]))
            ...
            pr <- prcomp(subset(cellDataCND, DC_IDs_FS == i)[,globalDimRedMarkers], 
                         center = TRUE, scale. = FALSE)
            colsCD = ncol(subset(cellDataCND, DC_IDs_FS == i)[,globalDimRedMarkers])
            NRSscore <- rowSums(outer(rep(1, colsCD),
                                      pr$sdev[1:3]^2) * abs(pr$rotation[,1:3]))
            
            NRSscoredf = tibble::rownames_to_column(as.data.frame(NRSscore), "marker") #store NRS values in df
            NRSscoredf$DC_IDs_FS = rep(i, nrow(NRSscoredf))
            masterNRSscores = rbind(masterNRSscores, NRSscoredf)
            NRSscoredf = subset(NRSscoredf, NRSscore > NRSthreshold) #filter markers with NRS scores above threshold
            ...
            if(length(deepClusterDimRedMarkers) < 2){ #if there are no non-redundant markers go to next iteration
              ...
              cellDataCND$temp[cellDataCND$DC_IDs_FS == i] = as.numeric(rep(1 + DCitrFS, length(cellDataCND$temp[cellDataCND$DC_IDs_FS == i]))) #append current PG cluster ID to temp column
              DCitrFS = DCitrFS + 1
              DCcounter = DCcounter + 1
              
              next
            }
            deepClusterdf = subset(cellDataCND, DC_IDs_FS == i) #store data of current PG cluster in dataframe
            deepClusterExprsdf = subset(cellDataCND, DC_IDs_FS == i)[,deepClusterDimRedMarkers] #create df containing markers of interest for deep clustering
            ...
            #deepClusterdf$deepCluster_meta <- meta_best[fsom$map$mapping[, 1]]
            deepClusterdf$DC_IDs_FS <- (meta_best[fsom$map$mapping[, 1]] + DCitrFS)
            DCitrFS = DCitrFS + optimal_k
            #cellDataCND$deepCluster_meta <- NA
            cellDataCND$temp[cellDataCND$DC_IDs_FS == i] = deepClusterdf$DC_IDs_FS
            deepClusterdf$DC_IDs_FS = factor(deepClusterdf$DC_IDs_FS)
            ...
            p4 <- ggplot(df_sil_deepCluster, aes(x = k, y = silhouette)) +
              ...
              geom_point(color = "darkred", size = 2) +
              geom_vline(xintercept = optimal_k, linetype = "dashed", color = "gray40") +
              labs(title = paste0("Silhouette Score FlowSOM cluster ",i),
                   x = "Number of Clusters (k)", y = "Avg Silhouette Score") +
              theme_classic(base_size = 14)
            print(p4)
            ggsave(paste0(dcfsdc,"/Silhouette score FlowSOM cluster_",i," DCround ", DCround, ".png"), device = "png")
            
            # === UMAP colored by final subclusters ===
            tempumapdf = as.data.frame(umap2(deepClusterExprsdf))
            colnames(tempumapdf) = c("DC_UMAP_1", "DC_UMAP_2")
            deepClusterdf$DC_UMAP_1 = tempumapdf[["DC_UMAP_1"]]
            deepClusterdf$DC_UMAP_2 = tempumapdf[["DC_UMAP_2"]]
            cellDataCND$DC_UMAP_1[cellDataCND$DC_IDs_FS == i] = tempumapdf$DC_UMAP_1
            cellDataCND$DC_UMAP_2[cellDataCND$DC_IDs_FS == i] = tempumapdf$DC_UMAP_2
            
            p5 <- ggplot(deepClusterdf, aes(x = DC_UMAP_1, y = DC_UMAP_2, color = DC_IDs_FS)) +
              geom_point(alpha = 0.6, size = 0.6) +
              #scale_color_viridis_d(name = "Subcluster") +
              labs(title = paste("UMAP on FlowSOM cluster ",i, " - optimal k ", optimal_k),
                   x = "DC_UMAP_1", y = "DC_UMAP_2") +
              theme_classic(base_size = 14)+
              guides(color = guide_legend(override.aes = list(size = 3)))
            
            if(length(unique(as.integer(deepClusterdf$DC_IDs_FS)))<27){
              p5 = p5 + scale_color_ucscgb()
            }else{
              p5 = p5 + scale_color_igv()
            }
            ...
            print(p5)
            ggsave(paste0(dcfsdc, "/UMAP FlowSOM ",i," DCround ", DCround,".png"), device = "png")
          ...
          cat("\n")
          cellDataCND$DC_IDs_FS = cellDataCND$temp
          cellDataCND$DC_IDs_FS = factor(cellDataCND$DC_IDs_FS)
          write.xlsx(masterNRSscores, paste0(dcfsdc, "/NRSscores FlowSOM DC round ", DCround,".xlsx"))
          DCitrFS = 0
          DCidsFS = as.numeric(cellDataCND$DC_IDs_FS)
        }
      ...
      if("PhenoGraph" %in% DCmethod){
        DCitrPG = 0
        DCidsPG = as.numeric(cellDataCND$dbScan_PG) #create a list of the current PhenoGraph cluster IDs
        cellDataCND$DC_IDs_PG = DCidsPG #create a column with the to-be final PhenoGraph cluster IDs
        cellDataCND$temp = DCidsPG #create a temporary column containing the cluster IDs after each while loop
        while(DCcounter != max(DCidsPG)){
          DCround = DCround + 1
          print(paste("Deep Clustering round", DCround))
          DCcounter = 0
          masterNRSscores = setNames(data.frame(matrix(ncol = 3, nrow = 0)), c("marker", "NRSscore", "DC_IDs_PG"))
          for(i in unique(cellDataCND$DC_IDs_PG)[order(unique(cellDataCND$DC_IDs_PG))]){
            cat("\n")
            print(paste("Analysing PhenoGraph cluster ",i, " of ", max(DCidsPG)))
            cat("\n")
            if(nrow(subset(cellDataCND, DC_IDs_PG == i)[,globalDimRedMarkers]) < DCminCells){ #if the minimum amount of cells in this cluster is not met deep clustering is skipped
              print("Amount of cells in this cluster is less than indicated lower limit. Skipping deepclustering")
              
              cellDataCND$temp[cellDataCND$DC_IDs_PG == i] = as.numeric(rep(1 + DCitrPG, length(cellDataCND$temp[cellDataCND$DC_IDs_PG == i]))) #append current PG cluster ID to temp column
              DCitrPG = DCitrPG + 1
              DCcounter = DCcounter + 1
            ...
            #Calculate NRS scores
            #pr <- prcomp(subset(cellDataCND, DC_IDs_PG == i)[,globalDimRedMarkers], 
                         #center = TRUE, scale. = FALSE)
            #NRSscore <- rowSums(outer(rep(1, length(globalDimRedMarkers)),
                                      #pr$sdev[1:length(globalDimRedMarkers)]^2) * abs(pr$rotation[,1:length(globalDimRedMarkers)]))
            pr <- prcomp(subset(cellDataCND, DC_IDs_PG == i)[,globalDimRedMarkers], 
                         center = TRUE, scale. = FALSE)
            colsCD = ncol(subset(cellDataCND, DC_IDs_PG == i)[,globalDimRedMarkers])
            NRSscore <- rowSums(outer(rep(1, colsCD),
                                      pr$sdev[1:3]^2) * abs(pr$rotation[,1:3]))
            
            NRSscoredf = tibble::rownames_to_column(as.data.frame(NRSscore), "marker") #store NRS values in df
            NRSscoredf$DC_IDs_PG = rep(i, nrow(NRSscoredf))
            masterNRSscores = rbind(masterNRSscores, NRSscoredf)
            NRSscoredf = subset(NRSscoredf, NRSscore > NRSthreshold) #filter markers with NRS scores above threshold
            ...
            deepClusterdf = subset(cellDataCND, DC_IDs_PG == i) #store data of current PG cluster in dataframe
            deepClusterExprsdf = subset(cellDataCND, DC_IDs_PG == i)[,deepClusterDimRedMarkers] #create df containing markers of interest for deep clustering
            
            Rphenograph_out <- Rphenograph(deepClusterExprsdf, k = PGnn) # perform phenograph
            PGclusters = as.numeric(membership(Rphenograph_out[[2]]) + DCitrPG) #store cluster IDs
            deepClusterdf$DC_IDs_PG = PGclusters #append clusters to deepcluster dataframe
            DCitrPG = DCitrPG + max(membership(Rphenograph_out[[2]])) #adjust PG clusters to follow increment of previous runs
            
            cellDataCND$temp[cellDataCND$DC_IDs_PG == i] = deepClusterdf$DC_IDs_PG #store new cluster IDs in temp column of master dataframe
            deepClusterdf$DC_IDs_PG = factor(deepClusterdf$DC_IDs_PG)
            ...
            deepClusterdf$DC_UMAP_1 = tempumapdf[["DC_UMAP_1"]]
            deepClusterdf$DC_UMAP_2 = tempumapdf[["DC_UMAP_2"]]
            cellDataCND$DC_UMAP_1[cellDataCND$DC_IDs_PG == i] = tempumapdf$DC_UMAP_1
            cellDataCND$DC_UMAP_2[cellDataCND$DC_IDs_PG == i] = tempumapdf$DC_UMAP_2
            
            
            p6 <- ggplot(deepClusterdf, aes(x = DC_UMAP_1, y = DC_UMAP_2, color = DC_IDs_PG)) +
              geom_point(alpha = 0.6, size = 0.6) +
              #scale_color_viridis_d(name = "Subcluster") +
              labs(title = paste("UMAP on PhenoGraph cluster ",i),
                   x = "DC_UMAP_1", y = "DC_UMAP_2") +
              theme_classic(base_size = 14)+
              guides(color = guide_legend(override.aes = list(size = 3)))
            
            if(length(unique(as.integer(deepClusterdf$DC_IDs_PG)))<27){
              p6 = p6 + scale_color_ucscgb()
            }else{
              p6 = p6 + scale_color_igv()
            }
            ...
            print(p6)
            ggsave(paste0(dcpgdc, "/UMAP PhenoGraph ",i," DCround ", DCround," - unmerged.png"), device = "png")
            
          }
          
          cat("\n")
          cellDataCND$DC_IDs_PG = cellDataCND$temp
          cellDataCND$DC_IDs_PG = factor(cellDataCND$DC_IDs_PG)
          write.xlsx(masterNRSscores, paste0(dcpgdc, "/NRSscores Phenograph DC round ", DCround,".xlsx"))
          DCitrPG = 0
          DCidsPG = as.numeric(cellDataCND$DC_IDs_PG)
      ...
      if(performMerge == TRUE){
        ...
        for(dbmerge in unique(cellDataCND$dbScan)){
          if(length(unique(as.numeric(subset(cellDataCND, dbScan == dbmerge)$DC_IDs_PG)))>1){
            ...
            exprsData = cbind(subset(cellDataCND, dbScan == dbmerge)$DC_IDs_PG, exprsData)
            exprsData = aggregate(exprsData[,-1], by = list(pgCluster = exprsData[,1]), FUN = median)
            pgCluster = exprsData$pgCluster
            exprsData = as.matrix(exprsData[, -1])
            colnames(exprsData) = colnames(cellDataCND[,globalDimRedMarkers])
            #rownames(exprsData) = 1:nrow(exprsData)
            rownames(exprsData) = pgCluster
            
            # Get all combinations of row indices
            row_pairs <- combn(nrow(exprsData), 2)
            
            # Subtract each pair: row i - row j
            diffs <- apply(row_pairs, 2, function(idx) {
              exprsData[idx[1], ] - exprsData[idx[2], ]
            })
            ...
            #obtain the medians of each marker of the differences
            if(mfiThresholdMethod == "1q"){
              medDiffs = apply(diffs, 2, summary)[2,]
            }else if(mfiThresholdMethod == "median"){
              medDiffs = apply(diffs, 2, summary)[3,]
            }else if(mfiThresholdMethod == "mean"){
              medDiffs = apply(diffs, 2, summary)[4,]
            }else if(mfiThresholdMethod == "3q"){
              medDiffs = apply(diffs, 2, summary)[5,]
            }else{
              stop("Invalid mfiThresholdMethod")
            }
            
            
            similarityList = vector(mode = "list", length = nrow(exprsData))
            tempLinks = vector(length = nrow(exprsData))
            startPos = 0
            for(clstrs in 1:(nrow(exprsData)-1)){
              tempLinks = vector(length = nrow(exprsData))
              tempLinks[1:clstrs] = NA
              nChecks = nrow(exprsData)-clstrs
              
              linkItr = clstrs + 1
              for(links in 1:nChecks){
                
                if(length(which(as.numeric(diffs[(links+startPos),]) > as.numeric(medDiffs))) > mergeThreshold){
                  isLink = 0
                }else{
                  isLink = 1
                }
                tempLinks[linkItr] = isLink
                linkItr = linkItr + 1
              }
              similarityList[[clstrs]] = tempLinks
              startPos = startPos+nChecks
            }
            similarityList[[nrow(exprsData)]] = rep(NA, nrow(exprsData))
            similarityMatrix1 = matrix(unlist(similarityList), nrow = nrow(exprsData), byrow = TRUE)
            similarityMatrix2 = t(similarityMatrix1)
            similarityMatrix = ifelse(is.na(similarityMatrix1), similarityMatrix2, similarityMatrix1)
            rownames(similarityMatrix) = colnames(similarityMatrix) = rownames(exprsData) 
            
            dir.create(paste0(dcr,"/Cluster_merging"))
            png(filename = paste0(dcr, "/Cluster_merging/Cluster Network Map - dbScan ", dbmerge,".png"), width = 3000, height = 3000, res = 300)
            plot(graph_from_adjacency_matrix(similarityMatrix, mode = 'upper'))
            #par(mar=c(1,1,1,1))
            dev.off()
            
            similarityMatrix = ifelse(is.na(similarityMatrix), 1, similarityMatrix)
            
            row_pairs2 <- combn(nrow(similarityMatrix), 2)
            diffs2 = apply(row_pairs, 2, function(idx) {
              similarityMatrix[idx[1], ] - similarityMatrix[idx[2], ]
            })
            diffs2 = abs(diffs2)
            
            similarityMatrix
            mergeIndex = which(colSums(diffs2) == 0)
            row_pairs2 = row_pairs2 + (min(as.numeric(rownames(exprsData))) - 1)
            mergeMatrix = as.matrix(row_pairs2[,mergeIndex])
            rmvcol = which(mergeMatrix[1,] %in% mergeMatrix[2,])
            if(length(rmvcol)>0){
              mergeMatrix = mergeMatrix[,-rmvcol]
            }
            print(paste0("merging table for dbScan ", dbmerge))
            print(mergeMatrix)
      ...
      #calculate percentages for each cluster and write excel file with the data
      percList = vector(mode = "list", length = length(unique(cellDataCND$sample_id)))
      idx = 0
      if("FlowSOM" %in% DCmethod){
        print("Writing FlowSOM cluster percentages")
        # for(sid in unique(cellDataCND$sample_id)){
        #   idx = idx+1
        #   tempList = c()
        #   tempList[1] = sid
        #   for(DCID in seq_along(unique(cellDataCND$DC_IDs_FS))){
        #     totalCells = nrow(subset(cellDataCND, sample_id == sid))
        #     clusterCells = nrow(subset(cellDataCND, sample_id == sid & DC_IDs_FS == DCID))
        #     tempList[DCID+1] = (clusterCells / totalCells) * 100
        #   }
        #   percList[[idx]] = tempList
        # }
        # clusterResults = as.data.frame(do.call(rbind, percList))
        # colnames(clusterResults) = c("sample_id", paste0(rep("FS", ncol(clusterResults)-1), 1:(ncol(clusterResults)-1)))
        # clusterResults = clusterResults %>% mutate_at(colnames(clusterResults[-1]), as.numeric)
        # write.xlsx(clusterResults, paste0(rsl, "/deepClusterResults_FS.xlsx"), row.names = FALSE)
        
        
        
        clusterPercents <- cellDataCND %>%
          group_by(sample_id, DC_IDs_FS) %>%
          summarise(clusterCells = n(), .groups = "drop") %>%
          group_by(sample_id) %>%
          mutate(totalCells = sum(clusterCells),
                 percent = round(((clusterCells / totalCells) * 100), digits = 2)) %>%
          ungroup()
        
        clusterResults <- clusterPercents %>%
          select(sample_id, DC_IDs_FS, percent) %>%
          pivot_wider(
            names_from = DC_IDs_FS,
            values_from = percent,
            names_prefix = "FS"
          )
        
        clusterResults[is.na(clusterResults)] <- 0
        clusterResults = as.data.frame(clusterResults)
        colTargetOrder = c("sample_id", paste0(rep("FS", (ncol(clusterResults)-1)), 1:(ncol(clusterResults)-1)))
        clusterResults = clusterResults[,match(colTargetOrder, colnames(clusterResults))]
        clusterResults$sample_id = as.character(clusterResults$sample_id)
      ...
      if("PhenoGraph" %in% DCmethod){
        ...
        if(performMerge == TRUE){
          print("Writing PhenoGraph cluster percentages for merged clusters")
          # print(Sys.time())
          # for(sid in unique(cellDataCND$sample_id)){
          #   idx = idx+1
          #   tempList = c()
          ...
          # }
          # clusterResults = as.data.frame(do.call(rbind, percList))
          # colnames(clusterResults) = c("sample_id", paste0(rep("PG", ncol(clusterResults)-1), 1:(ncol(clusterResults)-1)))
          # clusterResults = clusterResults %>% mutate_at(colnames(clusterResults[-1]), as.numeric)
          # write.xlsx(clusterResults, paste0(rsl, "/deepClusterResults_PG_merged.xlsx"), row.names = FALSE)
          # print(Sys.time())
          ...
          clusterResults[is.na(clusterResults)] <- 0
          clusterResults = as.data.frame(clusterResults)
          colTargetOrder = c("sample_id", paste0(rep("PG", (ncol(clusterResults)-1)), 1:(ncol(clusterResults)-1)))
          clusterResults = clusterResults[,match(colTargetOrder, colnames(clusterResults))]
          clusterResults$sample_id = as.character(clusterResults$sample_id)
        ...
        #print(Sys.time())
        # for(sid in unique(cellDataCND$sample_id)){
        #   print("Writing PhenoGraph cluster percentages for unmerged clusters")
        #   idx = idx+1
        #   tempList = c()
        #   tempList[1] = sid
        #   for(DCID in seq_along(unique(cellDataCND$DC_IDs_PG))){
        #     totalCells = nrow(subset(cellDataCND, sample_id == sid))
        #     clusterCells = nrow(subset(cellDataCND, sample_id == sid & DC_IDs_PG == DCID))
        #     tempList[DCID+1] = (clusterCells / totalCells) * 100
        #   }
        #   percList[[idx]] = tempList
        # }
        ...
        # colnames(clusterResults) = c("sample_id", paste0(rep("PG", ncol(clusterResults)-1), 1:(ncol(clusterResults)-1)))
        # clusterResults = clusterResults %>% mutate_at(colnames(clusterResults[-1]), as.numeric)
        # write.xlsx(clusterResults, paste0(rsl, "/deepClusterResults_PG.xlsx"), row.names = FALSE)
        # print(Sys.time())
        ...
        clusterResults <- clusterPercents %>%
          select(sample_id, DC_IDs_PG, percent) %>%
          pivot_wider(
            names_from = DC_IDs_PG,
            values_from = percent,
            names_prefix = "PG"
          )
        ...
        write.xlsx(clusterResults, paste0(rsl, "/deepClusterResults_PG_unmerged.xlsx"), row.names = FALSE)
      ...
      exprsData = as.matrix(cellDataCND[,(globalDimRedMarkers+length(globalDimRedMarkers))])
      ...
      print("Writing heatmap of dbScan clusters")
      exprsData = cbind(cellDataCND$dbScan, exprsData)
      assign("exprsDataPG" , exprsData, envir = .GlobalEnv)
      exprsData = aggregate(exprsData[,-1], by = list(dbCluster = exprsData[,1]), FUN = median)
      dbCluster = exprsData$dbCluster
      exprsData = as.matrix(exprsData[, -1])
      colnames(exprsData) = colnames(cellDataCND[,globalDimRedMarkers])
      rownames(exprsData) = dbCluster
      
      
      
      png(filename = paste0(dcr, "/Heatmap dbScan clusters.png"), width = 3000, height = 3000, res = 300)
      print(ComplexHeatmap::Heatmap(
        ...
        row_title = "Cluster_ID",
        row_title_side = "left",
        column_title = "Heatmap of dbScan clusters",
        column_title_side = "top",
        cluster_rows = FALSE,
      ...
      #heatmap of phenograph surface clusters  
      if("FlowSOM" %in% DCmethod){
        for(dbc in unique(cellDataCND$dbScan)){
          print(paste0("Writing heatmap of FlowSOM clusters for dbScan cluster " , dbc))
          ...
          exprsData = cbind(subset(cellDataCND, dbScan == dbc)$DC_IDs_FS, exprsData)
          exprsData = aggregate(exprsData[,-1], by = list(fsCluster = exprsData[,1]), FUN = median)
          fsCluster = exprsData$fsCluster
          exprsData = as.matrix(exprsData[, -1])
          colnames(exprsData) = colnames(cellDataCND[,globalDimRedMarkers])
          rownames(exprsData) = fsCluster
          
          png(filename = paste0(dcfsdc, "/Heatmap dbScan cluster ", dbc,".png"), width = 3000, height = 3000, res = 300)
          print(ComplexHeatmap::Heatmap(
            matrix = exprsData,
            name = "scaled expression",
            #column_title = "Title_test",
            ...
            row_title = "Cluster_ID",
            row_title_side = "left",
            column_title = "Heatmap of FlowSOM deepclusters",
            column_title_side = "top",
            cluster_rows = FALSE,
            cluster_columns = TRUE,
            show_row_dend = FALSE,
      ...
      if("PhenoGraph" %in% DCmethod){  
        if(performMerge == TRUE){
          for(dbc in unique(cellDataCND$dbScan)){
            ...
            png(filename = paste0(dcpgdc, "/Heatmap dbScan cluster ", dbc," - merged.png"), width = 3000, height = 3000, res = 300)
            print(ComplexHeatmap::Heatmap(
              matrix = exprsData,
              name = "scaled expression",
              #column_title = "Title_test",
              ...
              row_title = "Cluster_ID",
              row_title_side = "left",
              column_title = "Heatmap of PhenoGraph deepclusters",
              column_title_side = "top",
              cluster_rows = FALSE,
              cluster_columns = TRUE,
              show_row_dend = FALSE,
        ...
        for(dbc in unique(cellDataCND$dbScan)){
          ...
          print(paste0("Writing heatmap of unmerged phenograph clusters for dbScan cluster " , dbc))
          exprsData = as.matrix(subset(cellDataCND, dbScan == dbc)[,(globalDimRedMarkers+length(globalDimRedMarkers))])
          exprsData = cbind(subset(cellDataCND, dbScan == dbc)$DC_IDs_PG, exprsData)
          ...
          png(filename = paste0(dcpgdc, "/Heatmap dbScan cluster ", dbc,".png"), width = 3000, height = 3000, res = 300)
          print(ComplexHeatmap::Heatmap(
            matrix = exprsData,
            name = "scaled expression",
            #column_title = "Title_test",
      ...
      saveRDS(cellDataCND, file = paste0(rsl, "/cellDataCND.rds"))
      settingsList = c(paste0("Minimum cells per file: ", min_cells),
                       paste0("Maximum cells per file: ", max_cells),
                       paste0("Number of cells per file for UMAP: ", UMAP_cells),
                       paste0("Number of cells per file for TSNE: ", TSNE_cells),
                       paste0("Perform merge = " , performMerge),
                       paste0("DeepCluster threshold: " , DCminCells),
                       paste("NRS threshold: " , NRSthreshold),
                       paste("Number of bins: ", binNo),
                       paste0("dbScan epsilon: ", dbScanEps),
                       paste0("dbScan minimum neighborhood points: ", dbScanMinPts),
                       paste0("MFI threshold method: ", mfiThresholdMethod),
                       paste0("Merge threshold: ", mergeThreshold),
                       paste0("Plot fraction: ", plotFraction)
                       )
      fileConn = file(paste0(rsl, "/DC_settings.txt"))
      writeLines(settingsList, fileConn)
      close(fileConn)
    }  
  }
  ...
  plot_sequences_DR <- function() {
    ...
    # Create main folders
    main_folders <- c("00 Expression levels per marker in DR", "01 Cluster sequences", "02 Expression heatmap per cluster", "03 FlowSOM metacluster correlation matrices")
    ...
    MC_cor_folder = file.path(rsl, "00 Cluster number determination", "03 FlowSOM metacluster correlation matrices")
    ...
    for (nr in start_nr:end_nr){
      png(file.path(MC_cor_folder, paste0("MC", nr, " correlation matrix.png")), width = 7200, height = 6000, res = 300)
      print(createExprsMatrix(sce, k = paste0("meta",nr), bin_anno = TRUE, row_anno = TRUE, row_clust=TRUE, col_clust = FALSE, bars = TRUE, perc = TRUE, by=c("cluster_id"), fun = c("median")))
      dev.off()
    }
...
.anno_factors <- function(x, ids, which, type = c("row", "column")) {
  ...
  ComplexHeatmap::HeatmapAnnotation(which = type, df = df, 
...
.anno_clusters <- function(x, k, m, k_pal, m_pal) {
  ...
  ComplexHeatmap::rowAnnotation(df = df, col = col, gp = gpar(col = "white"))
}
...
.anno_counts <- function(x, perc) {
  ...
  if (perc) {
    ...
    foo <- ComplexHeatmap::row_anno_text(txt, 
                         just = "center", 
                         gp = gpar(fontsize = 8),  
                         location = unit(0.5, "npc"))
  } else foo <- NULL
  ComplexHeatmap::rowAnnotation(
    "n_cells" = ComplexHeatmap::row_anno_barplot(
...
createExprsMatrix <- function(x, features = NULL, 
  ...
  corMatrixdf = data.frame(corMatrix)
  corMatrixdf = rownames_to_column(corMatrixdf)
  colnames(corMatrixdf) = c("MC_Cor1", as.character(1:nrow(corMatrix)))
  colnames(corMatrixdf)
  corMatrixdf = pivot_longer(corMatrixdf, cols = 2:ncol(corMatrixdf), names_to = "MC_Cor2", values_to = "medianExprs")
  corMatrixdf$aboveThreshold = corMatrixdf$medianExprs > corThreshold
  corMatrixdf$MC_Cor1 = factor(corMatrixdf$MC_Cor1, levels = as.character(1:nrow(corMatrix)))
  corMatrixdf$MC_Cor2 = factor(corMatrixdf$MC_Cor2, levels = as.character(rev(1:nrow(corMatrix))))
  
  ggplot(corMatrixdf, aes(y = MC_Cor2, x = MC_Cor1, fill = aboveThreshold))+
    geom_tile()+
    geom_text(aes(label = round(medianExprs, 2)))+
    scale_fill_manual(values = c("white", "red"))+
    ggtitle(paste0("Correlation matrix - meta", nrow(corMatrix)))
...
cluster_new <- function(x, features = "type",
  ...
  ####CyCADAS####
  #cycFS = sce2fcs(sce, split_by = "sample_id", assay = "exprs", keep_cd = TRUE, keep_dr = TRUE)
  #sample_ids_cycadas = rep(md[(which(md$sample_id %in%  sampleNames(cycFS))),]$sample_id, fsApply(cycFS, nrow))
  ...
  #write.csv(expr_median, paste0(rsl, "/expr_median.csv"), row.names = F)
  #write.csv(df_prop, paste0(rsl, "/cluster_freq.csv"))
  
  ## ----------------------------------------------------------------------------
  ## Generate the Proportion Table
  ## ----------------------------------------------------------------------------
  #counts_table <- table(som$map$mapping[,1], sample_ids_cycadas)
  #props_table <- t(t(counts_table) / colSums(counts_table)) * 100
  
  #props <- as.data.frame.matrix(props_table)
  
  #write.csv(props, paste0(rsl,"/proportion_table.csv"))
